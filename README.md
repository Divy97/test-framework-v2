...
**GitHub has now accepted one.** On 2026-08-30 an issue filed on `Divy97/test-framework-v2-demo` reached a running engine through a real webhook delivery and came back as [a real pull request](https://github.com/Divy97/test-framework-v2-demo/pull/4) — **Tier 2, confidence 90/103**, `+13/-0` in `orders.mjs`. The recipe replayed (install, migrate, seed, and a service that answered its healthcheck), the reproduction ran red on base twice with the reported symptom, green three times on the fix, and the project's own suite passed on both commits. Four containers, 144 events, one App, no recording `fetch`.

Two things that run did **not** prove, and both matter. It went through `serve.ts` — the single-machine path — so **milestone 9's plane and runner are merged and tested but have never been deployed**. And the tunnel in front of it is a development tunnel that will be dead by morning, which is the entire argument for the control plane rather than a counterexample to it.

It also found what nothing in the suite could: the deployed `:latest` images were **two weeks old**, so the first delivery died in the environment build against an image that predates 7e. The suite builds `:test`; nothing had ever exercised what actually runs.

**Still not true, and this is the honest list:** the real run is a run, not a suite: one bug, one model, four attempts, and the other three seeded bugs have never been driven by a real agent. `/blobs` remains append-only by convention rather than construction. Diff-coverage is still not built, so an agent-authored reproduction still cannot earn Tier 1 — the real run capped at Tier 2 for exactly that reason. And the real run exposed one thing about the agent rather than the engine: `git_commit` stages everything, so the recipe's `npm install` left `package-lock.json` in the fix diff, which rule 4 of the fix prompt tells the agent not to do.
